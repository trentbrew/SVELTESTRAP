...
migrate(
	async (db) => {
		...
		const instructor = await dao.createRecord('users', {
		...
		const course1 = await dao.createRecord('courses', {
		...
		// Course 1 Modules
		const jsModule1 = await dao.createRecord('modules', {
			course: course1.id,
		...
		const jsModule2 = await dao.createRecord('modules', {
			course: course1.id,
		...
		const jsModule3 = await dao.createRecord('modules', {
			course: course1.id,
		...
		for (let i = 0; i < jsLessons.length; i++) {
			...
			await dao.createRecord('lessons', {
				course: course1.id,
				module: moduleId,
				title: lesson.title,
				slug: lesson.slug,
		...
		const course2 = await dao.createRecord('courses', {
		...
		// Course 2 Modules
		const reactModule1 = await dao.createRecord('modules', {
			course: course2.id,
		...
		const reactModule2 = await dao.createRecord('modules', {
			course: course2.id,
		...
		const reactModule3 = await dao.createRecord('modules', {
			course: course2.id,
		...
		const allLessons = await dao.getFullList('lessons');

		// Create exercises
		for (const exercise of exercises) {
			const lesson = allLessons.find((l) => l.slug === exercise.lessonSlug);
			if (lesson) {
				await dao.createRecord('exercises', {
					lesson: lesson.id,
		...
		const allExercises = await dao.getFullList('exercises');

		// Create checks
		for (const check of checks) {
			const exercise = allExercises.find((e) => e.title === check.exerciseTitle);
			if (exercise) {
				await dao.createRecord('checks', {
	...
	async (db) => {
		...
		for (const collectionName of collections) {
			const records = await dao.getFullList(collectionName);
			for (const record of records) {
				await dao.deleteRecord(collectionName, record.id);
			}
		}

		// Delete instructor user
		const users = await dao.getFullList('users', { filter: 'email = "instructor@academy.dev"' });
		for (const user of users) {
			await dao.deleteRecord('users', user.id);
		}
