<script lang="ts">
	...
	const columns: ColumnDef<Lesson>[] = [
		{
			accessorKey: 'title',
			header: 'Lesson Title',
			cell: ({ row }) => renderSnippet(LessonTitle, { row }),
			enableHiding: false
		},
		{
			accessorKey: 'estimatedMinutes',
			header: 'Duration',
			cell: ({ row }) => renderSnippet(LessonDuration, { row })
		},
		{
			id: 'progress',
			header: 'Progress',
			cell: ({ row }) => renderSnippet(LessonProgress, { row })
		},
		{
			accessorKey: 'isFree',
			header: 'Access',
			cell: ({ row }) => renderSnippet(LessonAccess, { row })
		},
		{
			accessorKey: 'status',
			header: 'Status',
			cell: ({ row }) => renderSnippet(LessonStatus, { row })
		},
		{
			accessorKey: 'order',
			header: 'Order',
			cell: ({ row }) => renderSnippet(LessonOrder, { row })
		},
		{
			id: 'actions',
			cell: () => renderSnippet(LessonActions)
		}
	];

	let { data, courseId, moduleId }: { data: Lesson[]; courseId: string; moduleId: string } =
		$props();
	...
	function handleLessonClick(lessonId: string) {
		goto(`/academy/courses/${courseId}/modules/${moduleId}/lessons/${lessonId}`);
	}
</script>
	...
	<div class="flex items-center justify-between px-4 lg:px-6">
		...
		<div class="flex items-center gap-2">
			<Button
				variant="outline"
				size="sm"
				onclick={() => goto(`/academy/courses/${courseId}/modules`)}
			>
					...
					{#if table.getRowModel().rows?.length}
						{#each table.getRowModel().rows as row (row.id)}
							<Table.Row
								...
								onclick={() => handleLessonClick(row.original.id)}
							>
...
{#snippet LessonTitle({ row }: { row: Row<Lesson> })}
	<div class="flex items-center gap-2">
...
{#snippet LessonDuration({ row }: { row: Row<Lesson> })}
	<div class="text-sm text-muted-foreground">
		{row.original.estimatedMinutes || 0} min
	</div>
{/snippet}

{#snippet LessonProgress({ row }: { row: Row<Lesson> })}
	<div class="w-24">
		<ProgressBar progress={row.original.progress || 0} size="sm" showPercentage={false} />
	</div>
{/snippet}

{#snippet LessonAccess({ row }: { row: Row<Lesson> })}
	<Badge variant={row.original.isFree ? 'default' : 'secondary'} class="px-1.5">
		{row.original.isFree ? 'Free' : 'Premium'}
	</Badge>
{/snippet}

{#snippet LessonStatus({ row }: { row: Row<Lesson> })}
	<Badge variant="outline" class="px-1.5 text-muted-foreground">
		{row.original.status}
	</Badge>
{/snippet}

{#snippet LessonOrder({ row }: { row: Row<Lesson> })}
	<Badge variant="outline" class="px-1.5 text-muted-foreground">
